...
#include "core/NamCodec.h"      // namz:: — .nam <-> .namz (embedded NAM currency = .namz)
...
namespace
{
    ...
    {
        if (mb.getSize() > 0 && ! namz::isNamz (mb.getData(), mb.getSize()))
            if (auto packed = namz::pack (mb.getData(), mb.getSize()); packed.getSize() > 0)
                return packed;
        return mb;
    }
...
juce::StringPairArray OrbitCabAudioProcessor::preampMetaFor (const juce::String& id) const
{
    ...
    return namz::readMeta (mb.getData(), mb.getSize());
}
            ...
            if (auto pool = root.getChildWithName ("PowerampPool"); pool.isValid())
                for (auto e : pool)
                {
                    ...
                    if (! juce::Base64::convertFromBase64 (decoded, b64))
                        continue;
                    if (namz::isNamz (decoded.getData(), decoded.getDataSize())
